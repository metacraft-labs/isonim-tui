...
##
## EX-M2 status: the canonical home for the entire `task_app/` tree
## (Layer-1 leaves + Layer-3 VM + Layer-2 view + Layer-4 composition
## roots) is now `isonim-examples/task_app/`. This test reaches into
## the sibling repo at `../../isonim-examples/task_app/` to assert:
##
##   1. There is exactly one `core/vm.nim` file under
##      `isonim-examples/task_app/`.
##   2. There is exactly one `core/views.nim` file under
##      `isonim-examples/task_app/`.
##   3. Both `tui/leaves.nim` and `web/leaves.nim` import that same
##      file (`task_app/core/vm`) — so they cannot drift.
##   4. A `diff` of the file against itself produces zero output (the
##      spec's literal phrasing).
##   5. The Layer-3 + Layer-2 files are renderer-pure: no
##      `isonim_tui` / `mock_dom` / `import ../tui` / `import ../web`
##      references.
##
## When a future contributor adds an additional copy under any sibling
## (e.g. an `isonim-website/` rendered demo), this test gains an
## additional check: `diff isonim-examples/task_app/core/vm.nim
## ../isonim-website/.../task_app/core/vm.nim` must produce zero
## output. Today only the canonical copy exists.
...
const
  ...
  taskAppDir =
    thisDir / ".." / ".." / "isonim-examples" / "task_app"
...
suite "M22: shared core is byte-identical across targets":
  test "test_task_app_shared_vm_byte_identical":
    ...
    # 2. There is no rival copy of vm.nim under isonim-examples/task_app/web/
    # or isonim-examples/task_app/tui/ — the cross-platform pattern
    ...
    # 2a. The pre-EX-M2 home in isonim-tui must be empty (the entire
    # examples/task_app/ tree was deleted as part of the EX-M2 cleanup).
    let oldRoot =
      thisDir / ".." / "examples" / "task_app"
    check not fileExists(oldRoot / "core" / "vm.nim")
    check not fileExists(oldRoot / "core" / "views.nim")
    check not fileExists(oldRoot / "tui" / "leaves.nim")
    check not fileExists(oldRoot / "web" / "leaves.nim")
    check not fileExists(oldRoot / "main_tui.nim")
    check not fileExists(oldRoot / "main_web.nim")

    # 3. Both leaves modules pin to `task_app/core/vm` (the canonical
    # path resolved by `--path:../isonim-examples` in isonim-tui's
    # config.nims, and by `--path:$config` in isonim-examples'
    # config.nims). The `import task_app/core/vm` text proves they
    # consume the shared VM rather than a local copy.
    let tuiLeaves = readFile(tuiLeavesPath)
    let webLeaves = readFile(webLeavesPath)
    check "import task_app/core/vm" in tuiLeaves
    check "import task_app/core/vm" in webLeaves
